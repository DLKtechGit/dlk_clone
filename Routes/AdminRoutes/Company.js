...
router.post("/createCompany", async (req, res) => {
  ...
  try {
    ...
    if (existingCompany) {
      return res.status(409).json({ message: "Company email already exist. Try another email." });
    }
...
router.post("/editCompany/:id", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Customer details updated successfully', company });
  } catch (error) {
